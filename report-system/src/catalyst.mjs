...
const evidenceClasses = new Set(["CONFIRMED", "COMPANY_GUIDANCE", "CONSENSUS", "MODEL_ESTIMATE", "UNKNOWN"]);
const confidenceValues = new Set(["LOW", "MEDIUM", "HIGH"]);
const summaryStatuses = new Set(["CATALYST_RICH", "SELECTIVE", "EXPECTATIONS_HIGH", "BINARY", "NO_CONFIRMED_CATALYST"]);
const catalystCategories = new Set(["EARNINGS", "PRODUCT", "CAPACITY", "R_AND_D", "REGULATORY", "CONTRACT", "PARTNERSHIP", "M_AND_A", "FINANCING", "POLICY", "LEGAL", "INDUSTRY", "OTHER"]);
...
export function validateCatalystReport(report) {
  ...
  push(
    "C04_FRAMEWORK",
    meta.framework_version === "1.0" ? "PASS" : "FAIL",
    "framework_versionは1.0",
  );
  ...
  const overallPricedOk = overall.quantifiable
    ...
    : overall.percent_range === null && overall.label === "定量不能";
  push(
    "C07B_SUMMARY",
    summaryStatuses.has(summary.status)
      && Boolean(summary.one_line && summary.next_window && summary.primary_risk && summary.surprise_up && summary.surprise_down)
      && overallPricedOk
      && confidenceValues.has(overall.confidence)
      && Boolean(overall.model)
    ? "PASS"
    : "FAIL",
    "要約・全体織り込み度の必須項目を確認",
  );
  ...
  for (const catalyst of catalysts) {
    ...
    push(
      ...
      Boolean(
        ...
        && catalyst.detail
        && catalyst.mechanism?.length
        && catalyst.impact_method
        && catalyst.impact_horizon
        && confidenceValues.has(catalyst.impact_confidence)
        && Array.isArray(catalyst.leading_indicators)
        && Array.isArray(catalyst.dependencies)
        && catalyst.sell_the_news_risk
        && catalyst.financing_risk
        && Array.isArray(catalyst.update_triggers)
      ) ? "PASS" : "FAIL",
      `${prefix}: カタリスト本体の必須項目を確認`,
    );
    push(
      `D01_${prefix}`,
    ...
    const labels = (catalyst.outcomes ?? []).map((outcome) => outcome.label).sort();
    push(
      `O01_${prefix}`,
      JSON.stringify(labels) === JSON.stringify(["FAILURE", "INLINE", "SUCCESS"]) ? "PASS" : "FAIL",
      `${prefix}: SUCCESS・INLINE・FAILUREを各1件`,
    );
    const probabilities = (catalyst.outcomes ?? []).map((outcome) => outcome.probability);
    ...
    for (const outcome of catalyst.outcomes ?? []) {
      const hasRanges = rangeIsValid(outcome.price_change_pct_range)
        && rangeIsValid(outcome.price_range, 0);
      push(
        `O03_${prefix}_${outcome.label}`,
        outcome.quantifiable ? (hasRanges ? "PASS" : "FAIL") : (!outcome.price_change_pct_range && !outcome.price_range ? "PASS" : "FAIL"),
    ...
    const pricedOk = priced.quantifiable
      ? rangeIsValid(priced.percent_range, 0, 100)
      : priced.percent_range === null && priced.label === "定量不能";
    push(
      `P01_${prefix}`,
      pricedOk
        && confidenceValues.has(priced.confidence)
        && Boolean(priced.model)
        && Array.isArray(priced.evidence_for)
        && Array.isArray(priced.evidence_against)
        ? "PASS"
        : "FAIL",
      `${prefix}: 織り込み度の幅・根拠・反証を確認`,
    );
    push(
      `S01_${prefix}`,
  ...
  push(
    ...
    report.validation_summary?.status === "FAIL" ? "FAIL" : "PASS",
    "内部ValidationがFAILではない",
  );
...
function renderPriced(priced) {
  if (!priced?.quantifiable || !rangeIsValid(priced.percent_range, 0, 100)) {
    return `<div class="priced unknown">定量不能<p>${h(priced?.model ?? "イベント単体価値を分離できません。")}</p></div>`;
  }
...
function renderCatalystCard(item, currency) {
  ...
  return `<article class="catalyst-card">
    ...
    <p>${h(item.detail)}</p>
    <div class="mechanism">${(item.mechanism ?? []).map((step, index) => `<span>${index ? '<i aria-hidden="true">→</i> ' : ""}${h(step)}</span>`).join("")}</div>
    <div class="outcomes">${(item.outcomes ?? []).map((outcome) => renderOutcome(outcome, currency)).join("")}</div>
    ${renderPriced(item.priced_in)}
    <div class="evidence"><div><h4>織り込みを支持する材料</h4>${list(item.priced_in?.evidence_for)}</div><div><h4>反証・不確実性</h4>${list(item.priced_in?.evidence_against)}</div></div>
    <div class="facts"><div class="fact"><span>影響の試算方法</span><b>${h(item.impact_method)}</b></div><div class="fact"><span>影響の時間軸</span><b>${h(item.impact_horizon)}｜確度 ${h(item.impact_confidence)}</b></div><div class="fact"><span>材料出尽くし</span><b>${h(item.sell_the_news_risk)}</b></div><div class="fact"><span>資金調達・希薄化</span><b>${h(item.financing_risk)}</b></div></div>
    ${extra}
...
function renderValidation(validation) {
  ...
  return `<p><b>${h(validation.status)}</b>｜FAIL ${validation.failCount}件｜WARN ${validation.warnCount}件</p>${exceptions.length ? list(exceptions.map((item) => `${item.id}: ${item.message}`)) : "<p>公開を止める検証エラーはありません。</p>"}`;
}
...
export function renderCatalyst(report, template, validation = validateCatalystReport(report)) {
  if (validation.status === "FAIL") throw new Error("Catalyst Validation FAILのためHTMLを生成しません。");
  ...
  const warnMessages = [
    ...
    ...(report.validation_summary?.messages ?? []),
  ];
  const html = renderTemplate(template, {
    ...
    OVERALL_PRICED_IN: h(overall.quantifiable ? formatRange(overall.percent_range, "%") : "定量不能"),
    OVERALL_PRICED_LABEL: h(overall.label),
    OVERALL_PRICED_BLOCK: renderPriced(overall),
    PRICED_IN_CONFIDENCE: h(overall.confidence),
    PRICED_IN_METHOD: h(overall.model),
