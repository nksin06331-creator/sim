...
function hasWaiver(report, id) {
  return (report.coverageWaivers ?? []).some((item) => item.checkId === id && item.reason && item.approvedBy && item.approvedAt);
}

function waived(report, check) {
  if (check.status !== "FAIL" || !hasWaiver(report, check.id)) return check;
  return { ...check, status: "WARN", message: `${check.message}（承認済み例外）` };
}
...
export async function validateReport(report, config) {
  const checks = [];
  const push = (id, status, message) => checks.push(waived(report, result(id, status, message)));
  ...
  push("S05_UPDATE_MODE", Boolean(report.update?.mode && report.update?.reason && report.update?.changedFields?.length) ? "PASS" : "FAIL", "更新理由と変更範囲を記録");
  ...
  if (meta.market === "US") {
    push(
      "M01_US_FIELDS",
      Boolean(meta.marketDetails?.reportingStandard && meta.marketDetails?.secCik !== undefined) ? "PASS" : "FAIL",
      "米国株は会計基準とSEC CIK確認",
    );
  ...
  if (report.renderMode === "structured") {
    ...
    push(
      "C02_SECTION_COUNT",
      sections.length >= config.coverage.new.minimumGuideSections ? "PASS" : "FAIL",
      `ガイドセクション ${sections.length}件`,
    );
    const glossaryCount = countItemsByBlock(report, "terms");
    push(
      "C03_GLOSSARY",
      glossaryCount >= config.coverage.new.minimumGlossaryTerms ? "PASS" : "WARN",
      `用語 ${glossaryCount}件`,
    );
    ...
    push("V04_EXPECTED_VALUE", Number.isFinite(shownExpected) && Math.abs(expected - shownExpected) <= Math.max(0.11, expected * 0.002) ? "PASS" : "FAIL", `期待値再計算 ${expected.toFixed(2)}`);
    const current = Number(scenario.valuation?.currentPrice);
    const expectedReturn = current ? (shownExpected / current - 1) * 100 : NaN;
    push(
      "V05_EXPECTED_RETURN",
      Number.isFinite(expectedReturn) && Math.abs(expectedReturn - Number(scenario.valuation?.expectedReturnPct)) <= 0.2 ? "PASS" : "FAIL",
      `期待リターン再計算 ${Number.isFinite(expectedReturn) ? expectedReturn.toFixed(1) : "不可"}%`,
    );
    ...
    push(
      "C04_CATALYSTS",
      catalysts.length >= config.coverage.new.minimumCatalysts ? "PASS" : "WARN",
      `カタリスト ${catalysts.length}件`,
    );
    const risks = scenario.risk?.items ?? [];
    push("C05_RISKS", risks.length >= config.coverage.new.minimumRisks ? "PASS" : "WARN", `リスク ${risks.length}件`);
    push("C06_POSITIVES", (scenario.positives?.length ?? 0) >= 3 ? "PASS" : "WARN", `良い点 ${scenario.positives?.length ?? 0}件`);
    push("C07_CONCERNS", (scenario.concerns?.length ?? 0) >= 3 ? "PASS" : "WARN", `懸念 ${scenario.concerns?.length ?? 0}件`);

    const sources = report.sources ?? [];
    const sourceIds = new Set(sources.map((source) => source.id));
    const primaryCount = sources.filter((source) => source.primary).length;
    push("SRC01_COUNT", sources.length >= config.coverage.new.minimumSources ? "PASS" : "WARN", `出典 ${sources.length}件`);
    push("SRC02_PRIMARY", primaryCount >= config.coverage.new.minimumPrimarySources ? "PASS" : "WARN", `一次情報 ${primaryCount}件`);
  ...
  push(
    ...
    "publishedには人の確認、またはV6機械公開ゲートPASSが必要",
  );
